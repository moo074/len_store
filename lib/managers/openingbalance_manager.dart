...
import 'package:len_store/forms/open_item_form.dart';
...
class OpeningBalanceManager extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return OpeningBalanceManagerState();
  }
}

class OpeningBalanceManagerState extends State<OpeningBalanceManager> {
  List<OpeningItem> _items = [];
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => OpeningItemForm(true)),
          ).then((value) => refresh());
        },
      ),
    );
  ...
  void refresh() async {
    ...
    List<Map<String, dynamic>> _results =
        await DBHelper.query(OpeningItem.table);
    ...
    setState(() {
      _items = _results.map((item) => OpeningItem.fromMap(item)).toList();
                          ...
                          onTap: () {
                            setState(() {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => OpeningItemForm(false,
                                        openingItem: _items[i])),
                              ).then((value) => refresh());
                            });
                          }));
                })
